...
let handle_call pid =
  ...
  let i =
    proc_pidinfo pid 13 (* PROC_PIDT_SHORTBSDINFO *)
      Unsigned.UInt64.zero (to_voidp bsd_shortinfo) (sizeof proc_bsdshortinfo)
  in
...
let () =
  ...
  else ignore (handle_call (PosixTypes.Pid.of_string Sys.argv.(1)))
