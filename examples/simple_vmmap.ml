...
let vm_region_submap_info_count_64 = sizeof (vm_region_submap_info_data_64_t) / sizeof (natural_t)
...
let vmmap task start_ end_ (depth : int) =
  ...
  while (!break == false) do
    ...
    if (not (Int32.equal kr kern_success) || (!@address) > end_) then begin
      ...
      let entry = mk_entry address_start address_end pr pw px ps offset
         device_major device_minor inode (pathname_str) in
      result := entry :: (!result);
  ...
  if Array.length Sys.argv <> 2 then
    Printf.fprintf stderr "Usage: %s <pid>\n" Sys.executable_name
      ...
      if (not (Int32.equal kr kern_success)) then (
        ...
        let maps = vmmap task (Unsigned.UInt64.zero) (Unsigned.UInt64.max_int) depth in
